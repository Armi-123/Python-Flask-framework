...
def update(id):
    if request.method == 'POST':
        ...
        Image = request.files["Image"]
        cursor = db.connection.cursor(MySQLdb.cursors.DictCursor)
        cursor.execute("UPDATE form SET First_Name = %s,Last_Name = %s,Address = %s,Mobile = %s,Dob = %s,Email = %s,Gender = %s ,State = %s , City = %s , Hobbies = %s,Education = %s,Image = %s WHERE id = %s",(First_Name,Last_Name,Address,Mobile,Dob,Email,Gender,[a],[b],Image.read(),City,id,))
        db.connection.commit()
        cursor.close()
        return redirect(url_for('data'))
